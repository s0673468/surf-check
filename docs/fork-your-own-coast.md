...
| # | File | What | Where |
|---|------|------|-------|
| 1 | `surf-config.js` | `BEACHES` — your spots | the `const BEACHES = [` array |
| 2 | `surf-config.js` | `SPOT_DATA_PROFILES` — optional per-spot physics | the `const SPOT_DATA_PROFILES = {` object |
| 3 | `app.js` | Map center + zoom | `.setView([-27.59, -48.46], 11)` |
| 4 | `surf-config.js` | Your timezone | `const TZ = "America/Sao_Paulo"` |
